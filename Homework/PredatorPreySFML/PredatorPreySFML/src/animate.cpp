	...
	: sidebar(WORK_PANEL, SIDE_BAR)
{
	...
	instruction = sf::Text("Enter: Step", font);
	instruction.setCharacterSize(20);
	instruction.setStyle(sf::Text::Bold);
	instruction.setFillColor(sf::Color::Green);
	instruction.setPosition(sf::Vector2f(myTextLabel.getPosition().x, myTextLabel.getPosition().y - 24));
...
void animate::update() {
	...
	myTextLabel.setString("Step: " + to_string(system.GetStep()));
}
